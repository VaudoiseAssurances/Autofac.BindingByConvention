...
namespace Autofac.BindingByConvention.FluentSyntax
{
    ...
    public class FluentContractFilter 
    {
        ...
        public FluentContractFilter(IRegistrationBuilder<object, ScanningActivatorData, DynamicRegistrationStyle> builder, params Func<Type, Type, bool>[] predicates)
        {
        ...
        public void Instanciated<T>() where T : RegistrationStrategyBase, new()
        {
            var strategy = new T();
            this.builder.As(implementationType =>
            {
                var enumerable = implementationType
                    .GetInterfaces()
                    .Where(interfaceType => ((FluentExceptInterfaces)this.Except).CheckAllFiltersSatisfied(interfaceType, implementationType))
                    .ToArray();
                return enumerable;
            });
